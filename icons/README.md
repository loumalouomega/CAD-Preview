...
## Application icon (`images/icon.svg`)

The editable source is [`../images/icon.svg`](../images/icon.svg): a teal extruded L bracket inspired by the Kratos block silhouette. Face shading, a circular bore with an offset inner wall, and a dimension line identify it as CAD geometry. The bore and surrounding canvas are genuinely transparent; there is no translucent badge. The matching simplified sidebar glyph is `../images/activity-icon.svg`.
