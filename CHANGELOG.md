...
- **fTetWild, a robust alternative volume-mesh engine for dirty triangle meshes.** Generating a tetrahedral mesh from an STL/OBJ/PLY/glTF source has always used Gmsh's own `classifySurfaces` path, which needs a watertight, manifold, well-oriented boundary — it throws or silently produces zero elements on the meshes people actually have (scans, sculpts, print-ready STLs with holes or self-intersections). A new **Engine** choice — Gmsh (default, unchanged) or fTetWild — is available in the FE Mesh panel, `generate_mesh`/`export_mesh`'s `options.engine`, and a new `repair_mesh` MCP tool that writes a repaired, watertight STL from a dirty source (tetrahedralize with fTetWild, keep the resulting volume mesh's own boundary) so `check_mesh_health`/`promote_mesh_to_brep` can then succeed where they previously reported an unclosable shape. fTetWild only ever applies to a mesh-format 3D source — a B-rep source or a non-3D `dimension` silently falls back to Gmsh with an explanatory warning.
