...
| `src/spaceMouse.ts` | Host-side SpaceMouse 6DOF reader (closed roadmap item): lazy `require("node-hid")`, usage-page-ranked discovery, report streaming with 3s reconnect, forwards raw motion to the active session (extension host only) |
...
| `src/tessellationTolerance.ts` | Pure half of mesh-aware tessellation export: `deriveTessellation` (cell size × fraction → absolute deflection, unit-preserving), `percentile`, binary STL writer (unit-tested) |
| `src/tessellationExport.ts` | Kernel half: re-tessellates the edited B-rep at the derived tolerance, writes binary STL, samples the achieved chordal error against the exact faces (`BRepExtrema_DistShapeShape` per sample point) — a `Pipeline` key (`exportTessellatedStl`) |
| `src/sheetSettings.ts` | Pure drawing-sheet settings resolver shared by the Export Drawing Sheet form and `export_drawing_sheet`: views, projection, paper, scale (number or ratio string), title, title-block fields; input > template > defaults, invalid values fall back with warnings (unit-tested) |
| `src/sheetTemplates.ts` | Pure sheet-template library parse/serialize/merge (bundled `dist/sheet-templates/starter-templates.json` + folder `cad-preview-sheet-templates.json`, caller wins) (unit-tested) |
| `src/drawingSheetForm.ts` | The Export Drawing Sheet settings form — a nonce-CSP standalone webview panel that only collects settings (template, views, projection, paper, scale, title-block fields, format); Save as template is delegated to the caller. `setTestSheetFormAnswer` is the integration-test seam |
| `src/meshDeviation.ts` | Pure CAD-to-mesh deviation core: deterministic area-weighted sampling, forward/reverse point-to-mesh distances over `triangleDistance.ts`'s grid, raw + Tukey-filtered stats, per-region failures, per-corner distances, PLY writer (unit-tested against analytic cases) |
| `src/meshDeviationHost.ts` | Kernel half: reference = the CAD's fine tessellation (per-face regions) or raw STL; mesh = a `generateMesh` run with the same inputs — a `Pipeline` key (`measureMeshDeviation`) |
| `src/passageAnalysis.ts` | Pure narrow-gap core: `findPassages` (coaxial-cylinder annuli with axial overlap, facing-plane slots with a true in-plane triangle overlap test, void-only via outward normals, requested size from Parts/global) (unit-tested) |
| `src/passageAnalysisHost.ts` | Kernel half: measures each face (analytic `faceSurfaceInfo`, orientation-aware winding normal, tessellation points) — a `Pipeline` key (`analyzePassages`) |
| `src/meshBudget.ts` | Pure, calibrated mesh size/memory estimate shared by the FE Mesh panel and `estimate_mesh_budget` (`estimateMeshBudget`, `budgetWarning`, formatters; raw calibration rows in `scripts/perf/mesh-budget-calibration.json`) (unit-tested against those rows) |
...
| `src/batchExport.ts` / `src/batchExportCommand.ts` | Pure batch orchestrator (`runBatch`, naming/collision rules, TSV + report HTML) and the session-free `cad-preview.batchExport` command, which runs `mcpTools.ts`'s `batchExportTool` over the host's kernel client under a `batch-<n>` owner and shows a read-only report panel |
| `src/html.ts` | `escapeHtml`, shared by every host-built HTML page |
| `src/handoffManifest.ts` / `src/kernelVersions.ts` | Pure simulation-handoff manifest (`buildHandoffManifest`, `checkHandoffManifest`, `replayFingerprint`) and the build-time-stamped kernel package versions it records. The facts come from `gmshService.ts`'s `computeHandoffFacts` (a Pipeline key); `mcpTools.ts`'s `buildExportHandoffManifest` is shared by `export_mesh` and the FE Mesh panel's Export |
| `src/prepReport.ts` / `src/prepReportCommand.ts` | Pure preparation-report documents (`renderPrepReportHtml` — escaped, script-free, images inlined only as PNG/JPEG data) and the `cad-preview.prepReport` command, which flushes the focused tab's sidecars, runs `mcpTools.ts`'s `generatePrepReportTool` under that document's job owner, and offers to open the HTML |
...
| `src/standardPartsThumbs.ts` | Host-side thumbnail fetching for the Standard Parts panel (roadmap Tier 1 "Standard-parts thumbnails", closed): `fetchThumbnail(url)` returns a `data:` URL or `null` on every failure/policy refusal (10 s timeout, 256 KiB cap on header AND stream, png/jpeg/gif/webp allowlist — never throws), plus a bounded session-scoped `ThumbCache` holding only successes. Unit-tested with an injectable fetch, zero real network I/O |
...
- **Jobs are document-scoped** (roadmap "Document-scoped jobs and cancellation"). The client keeps an explicit queue plus one running job; every job carries an optional `owner`. `withJob({owner, signal, timeoutMs})` returns a `ScopedPipeline` whose every call carries those options, plus `cancel()` for exactly that owner. `cancel({owner | jobId})` rejects a queued match with `JobCancelledError` WITHOUT ever sending it, and kills the child only when the RUNNING job matches — another owner's running job is never interrupted; queued jobs survive a kill and dispatch to the respawned child. An aborted `signal` cancels its job (the MCP server passes each request's `extra.signal` through an `AsyncLocalStorage` scope, so no handler changes were needed). `jobs()`/`onJobs()` expose the queued/running list; `cancelCurrent()` survives as "cancel whatever is running". `provider.ts` scopes every kernel call through `docPipeline(uri)` (owner = the document URI, timeout from `cadPreview.kernelTimeoutMinutes`); a load's Cancel and closing the tab both cancel that owner only.
- **`kernelClient.ts` also reports kernel readiness** (`kernelState()` / `onKernelState(listener)`): every call emits start / success / failure events into `kernelActivity.ts`'s pure reducer, and the child exiting (a cancel, the watchdog, a crash) emits a reset, since every kernel dies with it. `provider.ts` subscribes once in its constructor and fans each change out to every open session as a `kernelStatus` message (also sent in the `ready` handshake) — one child serves every document, so readiness is one fact for all of them. `mcpServer.ts` never subscribes.
- `runOwnedJob({ ownerId, requestId }, action)` scopes serialized kernel calls to one document/request. `jobStatus()` and `cancelOwnedJob()` require that same pair; queued calls are removed before worker dispatch, while a matching active call kills and replaces the kernel worker. The FE Mesh protocol echoes request IDs, and MCP queue-managed exports persist a separate atomic receipt so status lookup after server restart reports uncertainty instead of replaying the request.
...
`provider.ts` handles `massPropertiesRequest` for B-rep sources only (mesh sources compute the equivalent client-side in the webview — see [Webview API](./webview-api.md)); `mcpTools.ts`'s `get_mass_properties` additionally serves STL/OBJ/PLY/glTF headlessly via `src/triangleMassProperties.ts` + `src/meshInspection.ts` (triangle-based volume/area/centroid/`watertight`, no length or inertia), returning `{supported: false}` only for other mesh formats.
...
Three B-rep `vscode`-free functions sharing one private `resolveEntity()` id-resolver (`solid-N`/`face-N`/`edge-N`/`point-N`, via the already-shared `collectSolids`/`collectFaces`/`collectEdges`/`collectVertices` from `occtOperations.ts`) and the same read-parse-cleanup skeleton every other B-rep read path in this codebase follows — re-parse `bytes`, replay `ops`, resolve, compute, `.delete()` every handle in reverse order, `unlink` the MEMFS temp file. Backs the `inspect`/`measure`/`measure_exact` MCP tools (`mcpTools.ts`) and, for `measure_exact` only, the interactive webview's "⟟ Exact" measurement button (`measureExactRequest`, see [Protocol](./protocol.md) and [Webview API](./webview-api.md)). `inspect`/`measure` additionally serve STL/OBJ/PLY/glTF headlessly through `src/meshInspection.ts` (no OCCT — `measure_exact` stays B-rep-only).
...
interface ExactMeasureResult {
  ...
  axisDistance?: number              // "distance" only, two cylindrical faces — shortest infinite-axis separation
...
`provider.ts` handles `meshioOpsRequest` for meshio++-imported sources only (roadmap Tier 1 "Mesh-operations panel for meshio sources") — validates each entry via `src/meshioOps.ts`'s `validateMeshioOpSpec` (unknown op or out-of-range `ratio` rejects before touching WASM), then `handleMeshioOps()` reads the source bytes + companions (`resolveMeshioCompanionsFor`), calls the existing `runMeshioOps` pipeline entry (no new kernel surface — the same function `transform_mesh` drives headless), and writes the result to a NEW file via the shared `promptSaveAndWrite` (same-format extension, compound `.post.msh` preserved via `matchExtension`; OpenFOAM refused — its reader has no `readMesh` path). Returns the per-step report for the `meshioOpsResult` post (a dismissed save dialog posts nothing — a quiet no-op); per-step applied/skipped lines also surface as status messages.
...
Mesh-side region fitting (the `fit_mesh_region` feature) — `fit_mesh_region`. All three are pure and **dependency-free**, and the whole path is **WASM-free** (no `getOcct`), so it preserves the "opening a pure-mesh file must never load the WASM" invariant.
...
Per-solid primitive recognition (the "Primitive recognition report" feature) — `recognize_primitives`. Split three ways along this codebase's usual pure/impure line:
...
OpenSCAD `.csg` import (the OpenSCAD-support feature, path (a), closed) — see CLAUDE.md's own section for the full write-up, including the mid-session pivot from op-lowering to an opaque base and the probe trail. Split into a pure parser half and a kernel-side builder half, mirroring `entityFacts.ts`/`entityRebind.ts`'s own split.
...
**`saveCustomDocument(document)`** (Tier 0 Phase 2) — Ctrl+S / Save All / auto-save entry point. Flushes the sidecars (part of the save, not the whole of it), then bakes any unbaked tail for a B-rep source that can bake it — via the shared `bakeTailToSource("first")` the Export-menu save-in-place also uses (confirmed only until the session's first bake; afterwards the dirty dot + explicit keypress is the confirmation) — or `bakeMeshToSource("first")` for STL/OBJ/PLY (Tier 0 Phase 3). VS Code clears dirty on completion. Meshio/CAD-text sources flush sidecars only. Both bake paths pre-check `assertNotDirty` on the edits sidecar (a dirty sidecar fails the save before any write or modal), and roll the source back to its pre-save bytes when the watermark write throws after the source was already rewritten — so the pair agrees again instead of double-applying on reopen; the documented recovery for those two paths is simply saving again (roadmap "Save, reopen and recovery regression coverage", closed). Mesh sources are additionally covered by the transaction marker described below, which repairs a save that a *process death* interrupted rather than a thrown write.

### Recovering an interrupted mesh save-in-place

`bakeMeshToSource` and the headless `saveMeshModel` write the geometry and then advance the `bakedThrough` watermark. A process that simply stops between those two writes leaves the pair disagreeing — the file is baked, the sidecar is stale, and reopening replays the same edit over already-baked geometry. Rolling back on a *thrown* `writeEdits` (what both paths already do) does not cover that.

`<model>.save-journal.json` closes it. `bakeMeshToSource` opens the transaction with `beginMeshSave` immediately before touching the source (both hashes are computable there from bytes it already holds, and it also refreshes the one-deep `.bak` on the session's first save) and closes it with `endMeshSave` after the watermark lands; the headless `saveMeshModel` does the same, and additionally now writes through a temp sibling + rename like the interactive path, so a death mid-write can no longer truncate the source.

`recoverInterruptedMeshSave` (in the vscode-free `src/meshSaveRecovery.ts`, so both surfaces share one implementation) runs from two places: the `ready` hydration, **before** `readEdits`, so the normal read picks up whatever watermark recovery landed; and the top of `bakeMeshToSource`, so a new transaction never stacks on a stale one. Two ordering requirements exist only because of where the surrounding code sits, and are load-bearing:
...
3. Registers a `webviewPanel.webview.onDidReceiveMessage` listener (a per-panel `pending: Map<string, { resolve; reject }>` correlates export/screenshot round-trips, and separate per-panel debounce timers batch parts/annotations/edits/mesh-options sidecar writes)   that handles `"ready"`, `"partsChanged"`, `"annotationsChanged"`, `"editsChanged"`, `"meshingChanged"`, `"meshPresetApply"`, `"meshPresetSaveCurrent"`, `"meshPresetDelete"`, `"meshingGenerate"`, `"meshingExport"`, `"meshioOpsRequest"`, `"exportRequest"`, `"exportResult"`, `"exportError"`, `"screenshotButtonClicked"`, `"screenshotResult"`, `"screenshotError"`, `"massPropertiesRequest"`, `"bomRequest"`, `"clashCheckRequest"`, `"clashCheckAllRequest"`, `"primitiveRecognizeRequest"`, `"decomposeExportClicked"`, `"decomposeSaveMacroClicked"`, `"measureExactRequest"`, `"colorFieldRequest"`, `"standardPartsSearchRequest"`, `"standardPartsInsertRequest"`, `"standardPartsThumbsRequest"`, `"importSvgRequest"`, `"openFile"`, and `"openPath"`.
...
8a. On `"meshPresetApply"` / `"meshPresetSaveCurrent"` / `"meshPresetDelete"` (FE Mesh "Saved presets" row, roadmap Tier 1 "Reusable meshing presets"): resolves the merged user + bundled-starter library itself (same merge and precedence `sendMeshPresets` displays, so Apply and the panel list can never disagree). Apply converts units via `effectivePresetOptions()` (`src/meshPresets.ts`), writes `.mesh.json` + regenerated `.geo` (the `set_mesh_options` write exactly), updates `currentMeshOptions`, and re-posts `"meshingOptions"` so the panel re-renders — settings only, never a generate; warnings (unit conversion, engine-ignored fields) surface on the status line. Save prompts for a name (`showInputBox`, dismissed = quiet no-op) and stores the current mm-native options at `unit: "mm"` into the folder `cad-preview-mesh-presets.json` (the same file the MCP preset tools take as `libraryPath`, so agent and panel presets interoperate), then `sendMeshPresets()`. Delete refuses bundled starters (the panel hides the button; this is the backstop). `sendMeshPresets()` posts the `"meshingPresets"` list on `ready` and after every save/delete.
...
14a. On `"bomRequest"` (Parts-section "Copy BOM" button, roadmap Tier 1 "BOM Copy button"): B-rep sources only, same gate (else the `...Error` variant). Reads the sidecar Parts (zero rows + warning, not an error, when empty) and calls `computeBom()` (`src/massProperties.ts`, the same function `generate_bom` drives headless) over `replayTail(currentEdits, currentBakedThrough)`, posting `"bomResult"`/`"bomError"`. No caching, same as above.
14b. On `"clashCheckRequest"` / `"clashCheckAllRequest"` (Clash panel): B-rep sources only, same gate (else the `...Error` variant). Resolves Part names to `volumes` arrays in the handler (the pipeline functions stay Part-ignorant — the same split `checkInterferenceTool` owns headless); empty sides degrade to a clean no-overlap result without touching WASM. Calls `checkInterference()` / `checkInterferenceAll()` (`src/entityFacts.ts`) over `replayTail(currentEdits, currentBakedThrough)` and posts `"clashCheckResult"` / `"clashCheckAllResult"` (pairs named `partA`/`partB` in `i<j` order, with the `C(n,2)` contract guard) or the `...Error` variant. No caching, same as above.
14c. On `"primitiveRecognizeRequest"` (Primitives panel, Tier 1 "Primitive-recognition panel"): B-rep sources only, same gate (else the `...Error` variant). Calls `recognizePrimitives()` (`src/primitiveReport.ts`, the same function `recognize_primitives` drives headless) over `replayTail(currentEdits, currentBakedThrough)` and posts `"primitiveRecognizeResult"` / `"primitiveRecognizeError"`. No caching, same as above. On `"decomposeExportClicked"` / `"decomposeSaveMacroClicked"`: re-runs `recognizePrimitives`, computes the emission via `emitPrimitiveOps()` (same call shape `decomposeToPrimitivesTool` uses, including collision-renaming against the document's own variables) fresh in the handler rather than trusting a client snapshot; Export then runs the format/unit quick-picks + `promptSaveAndWrite()` around `buildPrimitivesFile()` (zero recognized solids throws instead of writing an empty file), Save-macro runs the name prompt + dry-compile probe + library write + `sendMacros()` (same file `macroSaveCurrent` writes, so agent and panel macros interoperate).
...
17b. On `"standardPartsThumbsRequest"` (roadmap Tier 1 "Standard-parts thumbnails", closed): fire-and-forget fetch of one rendered page's thumbnails — validates ids against the remembered `lastPartsSearch` (stale pages resolve nothing), fetches with concurrency 4 through the session `ThumbCache` (only successes cached/posted), and posts `"standardPartsThumbsResult"` with whatever succeeded (nothing posted when nothing did — text fallback, never an error). Runs detached from the message loop so a slow image never reads as a hung panel.
...
- Static view-controls panel HTML (`#view-controls`, `#vc-toggle`, including the `#display-mode-group` Display mode segmented buttons).
  - Sidebar (`#side`) containing the tree panel (`#tree-panel`), the Parts panel (`#parts-panel`), the Edits panel (`#edits-panel`), the FE Mesh panel (`#meshing-panel`), the Mass Properties panel (`#mass-panel`), the Clash panel (`#clash-panel`, B-rep sources only — hidden otherwise), and the Primitives panel (`#primitives-panel`, B-rep sources only — hidden otherwise).
...
## `src/meshParse.ts`, `src/meshSourceInput.ts`, `src/meshSweep.ts`, `src/meshEditBake.ts`
...
- **`meshSourceInput.ts`** — `resolveMeshSourceInput(route, sourcePath, ops, deps, warnings, unit?)` turns a mesh-format source (STL/OBJ/PLY/glTF, or any meshio++ format including OpenFOAM) into `{kind: "stl", stlBytes}` meshing input, scaled by `unit`. File reads and the two meshio conversions are injected (`deps`), so `mcpTools.ts`'s `resolveMeshInputHeadless` passes `node:fs` readers and its pipeline while `provider.ts` passes `vscode.workspace.fs` readers and the document-scoped kernel client. `isMeshSourceRoute(route)` says whether a route belongs here (everything except B-rep). The pending edit tail `ops` is baked in through the optional `deps.bakeEdits` (both callers pass the kernel worker's `bakeMeshEdits`; a meshio source is baked over its converted STL boundary, the `node-0` mesh the viewer edits). `bakeMeshSourceEdits`/`bakeWarnings` are exported for the other consumers (`compare_models`, drawings, `save_model`): the warnings are facts — `Baked N of M…`, one line per skipped op by index and kind — and a failed bake falls back to the raw file with a named warning.
  - **The `cad-preview.exportMesh` command uses it.** The command used to refuse every mesh-format source, because the extension got that geometry from the webview's serialized STL. It now resolves (and bakes) the source host-side before any quick-pick, posts the bake warnings as status lines, and runs the same `runMeshExport` chain as a B-rep source. The FE Mesh panel's own **Export** button still sends the displayed (edited) geometry.
- **`meshEditBake.ts`** (runs in the kernel worker; roadmap "Headless mesh-edit replay") — `bakeMeshEdits(bytes, format, ops, target, externalBuffers?) → {bytes, outcomes, messages}`, a `Pipeline` key (kernel `NONE`). Loads the source with the viewer's OWN three.js loaders (`webview/meshObject.ts`'s `parseMeshObject`/`parseGltfObject` — the same functions `meshLoaders.ts` uses, so the object hierarchy and every `node-N` id match), tags ids with the shared `tagMeshEntities`, runs `applyEditsMesh`, and serializes with `meshExporters.ts`'s `exportModel` (STL binary / OBJ / PLY). A host-side welded parse would have collapsed an OBJ group or a glTF node hierarchy and silently retargeted ops — which is why the loaders, not `parseToWeldedMesh`, build the scene. `requestAnimationFrame` (PLYExporter) and `ProgressEvent` (GLTFLoader) are polyfilled lazily; glTF strips materials/textures/images/samplers (no DOM image path) and inlines every buffer as a `data:` URI. Consumers: `resolveMeshSourceInput` (→ `generate_mesh`, `export_mesh`, `compare_mesh_refinement`, `measure_mesh_deviation`, the `exportMesh` command), `compare_models` and the drawing tools (baked side compared/drawn as STL), and `save_model` for STL/OBJ/PLY.
...
**Explicit external-change conflicts** (roadmap "Explicit external-change conflict handling"). Reconciliation used to be last-writer-wins in both directions — an external write overwrote in-memory state, and a pending debounced autosave then overwrote the external write. Now the five editable sidecars (edits, parts, planes, annotations, mesh options) go through a per-document `SidecarRevisionTracker` (`src/sidecarRevision.ts`): each kind has a KNOWN disk revision (sha256 of the bytes last read or written — seeded at open, refreshed on every own-write echo) and a PENDING flag (an autosave armed, not yet written). A watcher event whose content differs while a local change is pending — or a debounced autosave that finds disk moved away from the known revision — raises a conflict instead of writing: a `showWarningMessage` naming the kind, the file and both sides' counts, with **Reload from disk** (adopt the disk version, drop local) or **Keep mine (overwrite)**; dismissing leaves that kind's autosave paused (File ▸ Save is the explicit overwrite). A source-file replacement while unbaked edits exist asks **Reload / Keep editing** instead of reloading silently. View state stays last-writer-wins (a display preference). Honest limit: this is detection, not a transaction — a narrow window between the pre-write check and the write remains, and nothing is a cross-process lock; the watcher fires after such a race, so the loser is still visible. No merge of op lists or entity ids is attempted. The Test-only `simulateWebviewMessage(uri, msg)` delivers a message to the real handler so the integration suite can race a local edit against an external write.
