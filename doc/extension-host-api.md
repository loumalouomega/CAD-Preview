...
14a. On `"bomRequest"` (Parts-section "Copy BOM" button, roadmap Tier 2 "BOM Copy button"): B-rep sources only, same gate (else the `...Error` variant). Reads the sidecar Parts (zero rows + warning, not an error, when empty) and calls `computeBom()` (`src/massProperties.ts`, the same function `generate_bom` drives headless) over `replayTail(currentEdits, currentBakedThrough)`, posting `"bomResult"`/`"bomError"`. No caching, same as above.
