...
| `src/selectorQuery.ts` | Re-executable whole-bucket selector queries (Selector-synthesis ladder, rungs 1–2): `BucketSelector` shape (rung-2 optional `filter`/`rank`), tolerant `validateSelectorQuery` gate (a malformed induced layer fails the whole query, never a half-understood predicate), `isBindableSelector` (pattern-instance producers refuse), and `bucketReferenceIds` extraction (vscode/OCCT/THREE-free, unit-tested) |
