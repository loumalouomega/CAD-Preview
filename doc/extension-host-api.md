...
| `src/selectorQuery.ts` | Re-executable whole-bucket selector queries (Selector-synthesis ladder, rung 1): `BucketSelector` shape, tolerant `validateSelectorQuery` gate, `isBindableSelector` (pattern-instance producers refuse), and `bucketReferenceIds` extraction (vscode/OCCT/THREE-free, unit-tested) |
