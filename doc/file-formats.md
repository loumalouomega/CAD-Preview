...
1. **Read + convert** — `convertToStlBoundaryWithRegions(sourceBytes, format)` writes the raw file bytes into meshio++'s own virtual filesystem, `readMesh()`s the full mesh, and — when the file declares ≥1 named `kind: "cell"` region and the boundary comes back pure triangles — correlates each boundary triangle to its region via `extractSurface(mesh, recordParentIds=true)`'s `cell_data["surface:parent_cell"]` provenance array. Falls back to the plain `convertToStlBoundary()` (`convertSurface(inPath, outPath, {inFormat: format, outFormat: "stl"})`) with no region data for anything else (quad/hex boundaries, no regions, correlation finding nothing).
2. **Transport** — the resulting STL bytes are base64-encoded and posted as `{type: "loadMeshBytes", sourceFormat, dataBase64, regionAssignment?}`.
...
Only geometry (points + triangle connectivity) becomes actual geometry through this funnel. **Named `kind: "cell"` regions now DO become real Parts**, for the tetrahedral/triangular-boundary case: on first import (only when the parts sidecar is still empty — an existing Part is never overwritten), CAD-Preview auto-creates one selectable, colourable Part per region (`src/meshioRegionParts.ts`'s `buildPartsFromMeshioRegions`), persisted to the usual `<model>.parts.json` sidecar exactly like a manually-created Part. This works identically whether the file is opened in VS Code or loaded headlessly via the MCP server's `load_model` (one shared implementation, not two that could drift). Quad/hex volume boundaries, `kind: "point"`/`"side"` regions, and scalar point/cell/field data arrays (temperatures, stresses, …) are still **not** converted into anything you can select, colour, or export — their NAMES are surfaced informationally (a one-line status message on open, and a `load_model` warning via the MCP server), not their values. See CLAUDE.md's "meshio++ integration" section for the full mechanism and its exact scope. If you need to actually inspect scalar field values or colour by them, use a dedicated FE post-processing tool (e.g. ParaView) for these formats; CAD-Preview's support here is for quick geometry (and now region) previews alongside your CAD files.
