...
| Nastran Bulk Data | `.bdf` | routed as Nastran; current meshio++ reader rejects this extension's Gmsh-written deck | — |
...
Built: `cube`, `sphere`, `cylinder`, `polyhedron` (sewn, closure-checked), `union`/`difference`/`intersection`, `group`, `color` (transparent), `multmatrix`/`translate`/`rotate`/`scale`/`mirror` (all through one `gp_GTrsf` path — shear included, no decomposition needed), `linear_extrude`/`rotate_extrude` of a single `polygon`/`square`/`circle` child (2D `(x, y)` read as (radius, height) for revolve; `twist`/`scale` and `paths=` holes are a later phase and skip with a warning; `slices`/`convexity` are preview-only and ignored). Skipped with a `load_model` warning (whole subtree dropped): `hull`/`minkowski` (no OCCT equivalent), `text`/`import`/`surface` (external files/fonts), standalone 2D (`square`/`circle`/`polygon` outside an extrude), multi/union 2D extrude children, and anything unknown.
...
**Gmsh Mesh / Abaqus / I-DEAS Universal / SU2 / INRIA Medit close a real export/import asymmetry**: the FE Mesh panel already *wrote* `.msh`/`.inp`/`.unv`/`.su2`/`.mesh` via Gmsh's own writer, but until now had no way to re-*open* any of them — this codebase's own exported files were not importable. Each was verified round-trippable end-to-end against the live WASM (export a real tetrahedralized model, then `readMesh()` the result back) before being added. `.off`, which Gmsh also writes, was tried and **rejected** after the same check (`"Expected the first line to be 'OFF'"` — Gmsh's output isn't shaped the way meshio++'s OFF reader expects), so it is not an import format. **Nastran (`.bdf`) remains incomplete:** its route and `BEGIN BULK` normalization exist, but live verification shows meshio++ still rejects this extension's Gmsh-written deck as `Not a meshio++-C++ Nastran file`. It is not currently a verified import format; a parser or compatible reader is tracked in the roadmap. `.bdf` is also the X11 bitmap-font extension, so opening one shows a one-line caveat, like `.msh`/`.inp`.
...
**Mixed-topology XDMF round-trips with meshio++ 16.16.0.** The earlier `XDMF: unknown mixed topology index` reader defect is fixed upstream. Exports from this extension, including mixed point/line/triangle/tetra blocks, can be reopened and re-meshed when their referenced `.h5` companion is present. This is covered by the compatibility corpus and MCP smoke test.
...
User-defined **parts** (named groups of volumes / surfaces / lines) are stored in a JSON sidecar written **next to** the CAD file — e.g. `bull.stp` → `bull.stp.parts.json`. Part assignments never modify the CAD file itself — a Part edit writes only this sidecar. The sidecar is read on open (`readParts()`) and autosaved, debounced, on every edit (`writeParts()`), both in `src/partsStore.ts`. Parsing and serialization live in the vscode-free `src/partsSidecar.ts` so they are unit-tested.
...
- **Stored operand queries** are the id-field counterpart of `exprs`: `targetQueries` maps an operand field (`targets`, `a`/`b`, `edges`, `profile`, `faceA`, …) to a SelectorQuery (same shape `resolve_selector` takes), with `targetQueryKinds` carrying one producing-op-kind tag per bucket query. The id arrays stay the last-good caches; resolution is **host-side and replay-only** (`src/opQueryResolve.ts`) — the sidecar keeps queries + caches untouched, and every freeze (forward reference, kind-tag mismatch, pattern producer, unresolved/empty) keeps the cache and surfaces a warning. Mesh sources never resolve queries (they replay on cached ids); queries are refused inside `repeat` bodies.
...
| --- | --- | --- |
| B-rep (STEP/IGES/BREP) | host, OCCT (`applyEditsBRep`, `src/occtOperations.ts`) | every op kind: transforms, boolean, fillet/chamfer, extrude/revolve/sweep/loft (filled or thin-walled, optional region `pick`), shell/draft/defeature/splitByPlane/section/drill, explode/mate, all primitives (incl. wedge) and holes, all 2D profile sketches, all curves, and the bottom-up wireframe/build ops |
| Mesh (STL/OBJ/PLY/glTF) | webview, Three.js (`applyEditsMesh`, `src/webview/meshEdits.ts`) | translate, rotate, scale, mirror, boolean (via `three-bvh-csg`), explode, addBox, addSphere, addCylinder, addCone, addTorus, addPrism, and the hole family (addHole/addCounterboreHole/addCountersinkHole, via CSG subtraction) — everything else (fillet/chamfer, feature-modeling, shell/draft/defeature/split/section, mate, wedge, the 2D profile ops, curves, and the bottom-up wireframe ops) is B-rep only |
...
The 2D profile ops (`addCircleProfile`/`addRectangleProfile`/`addPolygonProfile`/ `addEllipseProfile`/`addRoundedRectangleProfile`/`addSlotProfile`/ `addTrapezoidProfile`) similarly need no operands, but build a **flat face** (no thickness) rather than a solid, and are B-rep only — their purpose is to be picked (Surf mode) as an `extrude`/`revolve`/`sweep`/`loft` profile afterward. A *curve* op's edges can be picked (Line mode) as a profile too, closed or open — see the sweep-family paragraph below. The curve ops (`addPolyline`/`addThreePointArc`/`addSpline`/`addBezier`/ `addEllipseArc`/`addHelix`) append standalone edges the same way the wireframe ops below do (B-rep only; `addSpline` is an approximating, endpoint-exact fit). The modify ops (`shell`/`draft`/`defeature`/`splitByPlane`/`section`/`drill`, B-rep only) reshape existing solids: shell hollows the solids owning the selected opening faces, draft tapers faces (currently a kernel-limited honest skip — see `doc/getting-started.md`), defeature removes faces as recognized features via `BRepAlgoAPI_Defeaturing` with the solid healing behind them, split cuts the targets by a plane keeping one or both sides, section appends the planar cross-section as a sketch face without touching the solids, and drill cuts the picked regions of a profile face/wire through the targets (one prism per region down an explicit `dir` × `length`). They're grouped together under a `"Sketches"` pseudo-body in the Components tree/view, made visible by a "free-face" pass in the tessellation pipeline (see `doc/extension-host-api.md`). Extruding/ revolving/sweeping/lofting a sketch consumes it into the resulting solid — it doesn't leave a duplicate face behind. **A thin-walled feature is the exception**: it builds its own band face from the sketch's outline and consumes that instead, so the sketch survives in `"Sketches"` and can be reused.

The four sweep-family ops (`extrude`/`revolve`/`sweep`/`loft`) accept an optional **`thin`** — the total wall thickness — to build a thin-walled body instead of a filled one, plus **`thinOuter`** for how much of that wall sits outside the profile boundary (`0`, the default, grows it entirely inward; `thinOuter === thin` entirely outward; anything between is a band straddling the outline). The profile is offset into that band before the ordinary builder runs, so the result is an ordinary solid in every other respect. A profile that already has a hole is refused rather than silently losing it.

They also take their profile in either of **two mutually exclusive forms** — exactly one must be present. `profile` (`profiles` for loft) is a `face-N`, as before. `profileEdges` (`profileEdgeSets` for loft, one set per section) is a set of `edge-N` ids assembled into a single wire, which is how an **open** sketch is consumed: an `addPolyline` with `closed: false` contributes one `edge-N` per segment, and those edges are the profile. The edges may be listed in any order — they are joined by their shared vertices — and a genuinely disconnected set is skipped with a diagnostic. A *closed* edge set behaves exactly like the equivalent face, filled or thin. An *open* one encloses no area, so it **requires** `thin`: its wall is a band centred on the spine with semicircular ends, which is also why `thinOuter` is refused there (an open wire has no inside or outside). Every section of a loft must agree on closedness.

The single-profile three (`extrude`/`revolve`/`sweep`, not `loft`) also accept an optional region **`pick`** narrowing which enclosed regions of a multi-region profile are consumed: a face's wires enumerate to regions (`0` = outer boundary, `1..N` = inner loops in order), an edge profile exposes a single region. Omitted (or `"outer"`) consumes the face as modeled, holes preserved; `"all"` — or any list containing `0` — consumes the outer boundary with every hole filled; a list without `0` builds each picked inner loop as a standalone island body. An out-of-range index skips with a diagnostic naming the loop count, and an explicit `pick` refuses `thin` (thin walls build from the outer boundary alone). `drill` cuts the same picked regions through target solids instead of appending them: one prism per region down an explicit `dir` × `length`, subtracted from the targets (no up-to-face variant).

Both fields are ordinary numeric op fields, so a parametric variable can drive them like any other — sketch a rectangle, then extrude it as a tube whose wall is a variable:

```parametric
{
  "variables": [{ "name": "wall", "expr": "2" }],
  "steps": [
    { "op": { "op": "addRectangleProfile", "center": [0, 0, 0], "normal": [0, 0, 1], "up": [1, 0, 0], "width": 40, "height": 25 } },
    { "op": { "op": "extrude", "profile": "face-6", "dir": [0, 0, 1], "length": 30, "thin": 2, "exprs": { "thin": "wall" } } }
  ]
}
```

The bottom-up wireframe ops (`addPoint`/`addLine`/`addArc`/`addSurfaceFromLines`/ `addVolumeFromSurfaces`) let a shape be built up the traditional CAD way instead of from parametric primitives or profile extrusion: place standalone points, connect them with lines/arcs (typed endpoints, not point references), select a closed set of lines and **Build → Surface** to assemble a face, then select a closed set of surfaces and **Build → Volume** to sew them into a solid. **`addSurfaceFromLines` also accepts SEVERAL disjoint closed loops in one selection, as long as they form exactly one outer boundary plus its holes** (roadmap "3D text via outline import") — a letter with a counter, e.g. an imported "O"'s outer ring and inner ring, builds as one holed face rather than needing a manual boolean; a selection spanning more than one outer loop (two unrelated letters, say) is refused with a message naming the region count, since each region is its own op. Like the 2D profile ops, all five are B-rep only — meshes have no wire/sewing concept — and a point-select mode (`📍 Point`) shows every vertex in the model (original geometry's corners as well as added points), consistent with how Vol/Surf/Line already show everything. An `addSurfaceFromLines`/`addVolumeFromSurfaces` selection that doesn't actually close (open chain of lines, open shell of surfaces) is skipped gracefully — no error, no op applied.
...
`sidebarWidth` (roadmap "Sidebar layout and keyboard usability") is a fourth such optional sibling: the resizable sidebar's width in px. Read values are clamped through `clampSidebarWidth` (`src/webview/sidebarResizer.ts`, range 176..420) — a hand-edited out-of-range number falls back to the clamp bounds, never the raw value, and a non-numeric value means "unpersisted". Absent **or exactly 220** (the pre-resizer fixed width) writes nothing, so an unresized document's sidecar stays byte-stable. Same shape, same no-version-bump rule, same exclusion from the Preprocess Archive and from MCP as `collapsedPanels`.
