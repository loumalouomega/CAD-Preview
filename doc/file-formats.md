...
The four sweep-family ops (`extrude`/`revolve`/`sweep`/`loft`) accept an optional **`thin`** — the total wall thickness — to build a thin-walled body instead of a filled one, plus **`thinOuter`** for how much of that wall sits outside the profile boundary (`0`, the default, grows it entirely inward; `thinOuter === thin` entirely outward; anything between is a band straddling the outline). The profile is offset into that band before the ordinary builder runs, so the result is an ordinary solid in every other respect. Closed face profiles only, and a profile that already has a hole is refused rather than silently losing it.
