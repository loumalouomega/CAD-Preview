...
| --- | --- | --- |
| B-rep (STEP/IGES/BREP) | host, OCCT (`applyEditsBRep`, `src/occtOperations.ts`) | every op kind: transforms, boolean, fillet/chamfer, extrude/revolve/sweep/loft (filled or thin-walled, optional region `pick`), shell/splitByPlane/section/drill, explode/mate, all primitives (incl. wedge) and holes, all 2D profile sketches, all curves, and the bottom-up wireframe/build ops |
