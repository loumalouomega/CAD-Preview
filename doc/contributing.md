...
- **B-rep export targets are written in the host via OCCT; mesh export targets are written in the webview via Three.js exporters.** This build of OCCT has no STL/OBJ/PLY/glTF writers and no path from a mesh back to a B-rep — don't try to route mesh-to-B-rep export through OCCT, and don't move B-rep writing into the webview (same CSP/WASM reasoning as reading). **The one carve-out is silhouette SVG export** (`src/svgSilhouetteHost.ts`, **File ▸ Export Silhouette SVG…**): it writes a 2D drawing, not a CAD or mesh document, so it is neither kind of target, is written in the host for *both* source families, and must stay out of `CadFormat`/`exportTargetsFor()` — adding an `"svg"` format member would ripple into `package.json`'s `customEditors.selector` and make VS Code open `.svg` files in the 3D viewer, colliding with Import SVG…. A future non-CAD output format (a DXF outline, say) belongs beside it, not inside the two-case split.
