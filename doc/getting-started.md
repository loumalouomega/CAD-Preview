...
- **Export offers STEP/IGES and the mesh formats, plus the source's own format as a confirmed save-in-place** — the export list excludes a document's own format only for the formats with no same-format writer (glTF, meshio++, CAD-text). Picking your own STEP/IGES/BREP/STL/OBJ/PLY writes the baked model back to the open file (with `.bak` + history watermark); export anything else for a file that carries the geometry itself.
...
| **Export…** | Convert the model to a compatible format and save it — or pick the source's own format to save the edits back into the open file itself (confirmed, with a `.bak` backup) — see [Exporting a Model](#exporting-a-model) | Ctrl+E |
...
- **Source CAD file is written only by an explicit save.** Ctrl+S (or Save All / auto-save) on a dirty tab bakes the unbaked op tail into the open STEP/IGES/BREP/STL/OBJ/PLY file itself (confirmed until the session's first bake, then on the dirty dot alone; temp-file + rename, one-deep `.bak`, history watermark — undo cannot cross the save point, and `File: Revert File` drops back to it). **Export** to another format writes a new, separate file; picking the source's own format in the Export flow saves in place behind its own modal confirmation. **Part** definitions are saved to a `<model>.parts.json` sidecar; **pinned measurements** to a `<model>.annotations.json` sidecar; and **edit operations** to a `<model>.edits.json` sidecar — the original geometry is otherwise always left untouched. Edits are non-destructive and replayable, and are baked in on **save** or **Export**.
- **An interrupted mesh save is repaired on the next open, but a B-rep one is not.** A save writes the geometry and then advances the edit history's save watermark. If the process stops between those two writes, the file is baked while the history is stale — and reopening would replay the same edit over geometry that already contains it. For **STL/OBJ/PLY** a `<model>.save-journal.json` transaction marker makes that recoverable: on the next open the save is either completed (the file provably holds the saved geometry) or discarded (the file was never rewritten), and if it matches neither — a half-written file, or something else editing it in the meantime — nothing is guessed and you are asked whether to restore `<model>.bak`. For **STEP/IGES/BREP** there is no such marker: the only protection is that a *failed* save rolls the file back, so a hard crash (kill, power loss) in that window still needs you to save again. See [Exporting a Model](#exporting-a-model).
