...
- **Source CAD file is written only by an explicit, confirmed save-in-place.** **Export** to another format writes a new, separate file; picking the source's own STEP/IGES/BREP format writes the baked model back to the open file itself (modal confirmation, temp-file + rename, one-deep `.bak`, history watermark — undo cannot cross the save point). **Part** definitions are saved to a `<model>.parts.json` sidecar; **pinned measurements** to a `<model>.annotations.json` sidecar; and **edit operations** are saved to a `<model>.edits.json` sidecar — the original geometry is otherwise always left untouched. Edits are non-destructive and replayable, and are baked in on **Export** or a save-in-place.
