...
`save_preprocess`/`load_preprocess` package/restore the CAD source plus whichever of the `.edits.json`/`.parts.json`/`.mesh.json`/`.geo` sidecars exist on disk as a single portable `.zip` — a missing sidecar (e.g. mesh options never set) is simply omitted from the archive, never an error. `<model>.annotations.json` is **not** currently included (scoped into the still-open "Archive integrity" roadmap item's own schema/versioning work). The `.geo` script inside the archive is never restored verbatim on `load_preprocess`; the mesh options sidecar (if any) is re-written through the normal options path instead, which regenerates `.geo` fresh — same one-way-generation rule as every other write path.
