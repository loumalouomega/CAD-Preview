...
- **Lazy WASM init, mirroring OCCT's.** Never call the factory in `activate()`.
  ...
  **▶ Generate** or an **Export .msh**/**Export .geo** button, never on file open —
...
- **The `.geo` file is ONE-WAY generated — hand-edits are never read back.** On
  ...
  is what the panel's **Export .geo** button actually produces
  (`exportGeoUnrolled` in `gmshService.ts`), a *different* file from the
  autogenerated sidecar `.geo`. The sidecar `.geo`'s own header comment says as
  much; any manual edit to it is silently overwritten on the next options change.
