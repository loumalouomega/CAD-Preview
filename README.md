...
- **Measurement tools**: measure distance, edge length, angle, and circle/arc radius directly in the 3D view — a display-only overlay, never an edit operation; a completed result can optionally be **pinned** (📌) as a persisted, topology-anchored **annotation**, saved to a `<model>.annotations.json` sidecar and automatically re-matched to its anchored entity across topology-changing edits — the same best-effort geometric rebinding Parts already get.
